...
**CASO DE USO:** COTA CAPITAL DAS FILIAIS

## 1. Introdução
Este documento detalha o endpoint desenvolvido em AdvPL para obter o saldo de cota capital das filiais. O objetivo é permitir que outros sistemas consumam esses dados por meio de uma integração RESTful com o Protheus.
...
```advpl
WSRESTFUL ebarn_filiais DESCRIPTION "obter o saldo de cota capital das filiais"
```
...
O retorno é em JSON e traz informações detalhadas sobre cota capital das filiais:

| Parâmetro            | Tipo   | Descrição                             |
|----------------------|--------|-------------------------------------|
| filialId             | string | ID da filial                        |
| cotaCapital          | float  | Saldo de cota capital da filial     |
...
Static function MainFunction( CnpjCpf )
    ...
    BeginSQL Alias cQRY
        SELECT filialId, cotaCapital, dataUltimaAtualizacao
                           FROM %table:filiais%
