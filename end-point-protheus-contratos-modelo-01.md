...
## 1. Introdução
Este documento detalha o endpoint desenvolvido em AdvPL para retornar contratos de depósito de terceiros. O objetivo é permitir que outros sistemas consumam esses dados por meio de uma integração RESTful com o Protheus.
...
```advpl
WSRESTFUL ebarn_ctrsDepTerceiros DESCRIPTION "retornar contratos de depósito de terceiros"
```
...
```bash
curl -X GET 'https://api.protheus.com.br/v1/ebarn_ctrsDepTerceiros?CnpjCpf=12345678910'
```
...
O retorno é em JSON e traz informações detalhadas sobre contratos de depósito de terceiros:

| Parâmetro            | Tipo   | Descrição                             |
|----------------------|--------|-------------------------------------|
| contratoId           | string | ID do contrato                      |
| cooperadoId          | string | ID do cooperado                     |
| dataContrato         | string | Data de criação do contrato         |
...
Static function MainFunction( CnpjCpf )
    ...
    BeginSQL Alias cQRY
        SELECT contratoId, cooperadoId, dataContrato
                           FROM %table:contratos_terceiros%
