...
**CASO DE USO:** CADASTRO DE CATEGORIAS DE PRODUTOS

## 1. Introdução
Este documento detalha o endpoint desenvolvido em AdvPL para retornar as categorias de produtos e seus produtos. O objetivo é permitir que outros sistemas consumam esses dados por meio de uma integração RESTful com o Protheus.
...
```advpl
WSRESTFUL ebarn_produtos DESCRIPTION "retornar as categorias de produtos e seus produtos"
```
...
```bash
curl -X GET 'https://api.protheus.com.br/v1/ebarn_produtos?CnpjCpf=12345678910'
```
...
O retorno é em JSON e traz informações detalhadas sobre cadastro de categorias de produtos:

| Parâmetro       | Tipo    | Descrição                               |
|-----------------|---------|-----------------------------------------|
| categoriaId     | string  | ID da categoria de produto             |
| categoriaNome   | string  | Nome da categoria                      |
| produtoId       | string  | ID do produto                          |
| produtoNome     | string  | Nome do produto associado à categoria  |
...
Static function MainFunction( CnpjCpf )
    ...
    BeginSQL Alias cQRY
        SELECT DISTINCT categoriaId, categoriaNome, produtoId, produtoNome
                           FROM %table:categorias_produtos%
