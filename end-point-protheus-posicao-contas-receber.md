...
**CASO DE USO:** POSIÇÃO FINANCEIRA DE CONTAS A RECEBER

## 1. Introdução
Este documento detalha o endpoint desenvolvido em AdvPL para retornar a posição financeira de contas a receber. O objetivo é permitir que outros sistemas consumam esses dados por meio de uma integração RESTful com o Protheus.
...
```advpl
WSRESTFUL ebarn_posContasReceber DESCRIPTION "retornar a posição financeira de contas a receber"
```
...
```bash
curl -X GET 'https://api.protheus.com.br/v1/ebarn_posContasReceber?CnpjCpf=12345678910'
```
...
O retorno é em JSON e traz informações detalhadas sobre posição financeira de contas a receber:

| Parâmetro       | Tipo    | Descrição                                    |
|-----------------|---------|----------------------------------------------|
| cnpjCpf         | string  | CPF ou CNPJ do cliente                       |
| saldoDevedor    | float   | Saldo total devedor em aberto                |
...
Static function MainFunction( CnpjCpf )
    ...
    BeginSQL Alias cQRY
        SELECT cnpjCpf, saldoDevedor, dataUltimaAtualizacao
                           FROM contas_receber
